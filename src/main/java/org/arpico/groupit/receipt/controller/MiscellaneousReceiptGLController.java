...
@RestController
@CrossOrigin(origins = "*")
public class MiscellaneousReceiptGLController {

	@Autowired
	private MiscellaneousReceiptGLService miscellaneousReceiptGLService;
	...
	public List<AccountGLDto> getAccounts () throws Exception {
		...
		return miscellaneousReceiptGLService.getAccounts();
	}
	
	@RequestMapping(value = "/misglreceiptsave/{token:.+}", method = RequestMethod.POST)
	public ResponseEntity<Object> saveReceipt (@RequestBody MiscellaneousReceiptInvDto dto, @PathVariable String token) throws Exception{
		...
		String validity = commonValidations.validateMiscellaneousReceiptGlInputs(dto, token);
		
		if(validity.equalsIgnoreCase("ok")){
			return miscellaneousReceiptGLService.save(dto, token);
		} else {
	...
	@RequestMapping(value = "/recmlast", method = RequestMethod.POST)
	public List<RmsRecmDto> getLastReceipts(@RequestParam String token) throws Exception{
		return miscellaneousReceiptGLService.getLatestReceipts(token);
	}
}
