				...
				if (receiptCancelationDao.save(canceledReceiptModel) != null) {
					...
					if (toEmail != null && toEmail != "" && fromEmail != null && fromEmail != "") {
						...
						emailDto.setSubject("Receipt Cancelation Approval ("
								+ inTransactionsModel.getInTransactionsModelPK().getDoccod() + "/" + receiptNo + " - "
								+ branchName + ")");

						body += " Receipt Number : " + inTransactionsModel.getInTransactionsModelPK().getDoccod() + "/"
								+ receiptNo + "\n";
...
=======
		...
		List<CanceledReceiptModel> canceledReceiptModels=receiptCancelationDao.findByStatusAndReceiptNo("PENDING", receiptNo);
...
//									body += "This is system generated email. Do not reply.";
			...
			if (inLoanTransactionsModel != null) {
				...
				if (userCode != null) {
					...
					canceledReceiptModel.setRequestBy(userCode);
					canceledReceiptModel.setRequestDate(new Date());
					canceledReceiptModel.setLocCode(inLoanTransactionsModel.getInLoanTransactionsModelPK().getLoccod());
					canceledReceiptModel.setSbuCode("450");
					canceledReceiptModel.setStatus("PENDING");
					canceledReceiptModel.setDocCode(inLoanTransactionsModel.getInLoanTransactionsModelPK().getDoccod());
