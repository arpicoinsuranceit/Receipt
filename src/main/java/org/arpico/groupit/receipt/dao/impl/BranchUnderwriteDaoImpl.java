...
public class BranchUnderwriteDaoImpl implements BranchUnderwriteDao {
	...
	public List<InProposalUnderwriteModel> findProposalToUnderwrite(String locodes, Integer limit, Integer offset,
		...
		if(isHO) {
			return jdbcTemplate.query("select p.pprnum,p.prpseq,p.polnum,p.cscode,p.ppdnam,p.advcod, p.loccod as brncod,p.loccod,p.ppdnic,p.prdcod "
					...
					+ " where  p.sbucod = '450'  and p.pprsta = 'L0' order by cast(p.pprnum as unsigned) desc LIMIT "+limit+" OFFSET "+offset+";",
					new InProposalUnderwriteRowMapper());
		}else {
			return jdbcTemplate.query("select p.pprnum,p.prpseq,p.polnum,p.cscode,p.ppdnam,p.advcod, p.loccod as brncod,p.loccod,p.ppdnic,p.prdcod "
					+ "FROM inproposals p inner join inagentmast a on p.sbucod = a.sbucod and p.advcod = a.agncod"
					+ " where p.sbucod = '450' and p.loccod in ("+locodes+") and  p.pprsta = 'L0' order by cast(p.pprnum as unsigned) desc LIMIT "+limit+" OFFSET "+offset+";",
					new InProposalUnderwriteRowMapper());
		}
	...
	@Override
	public List<String> findLocCodesZonalBranch(String zoneCodes,String userType) {
		List<String> userLocList = null;
		String sql="";
		if(userType.equals("REGION")) {
			sql="select l.loccod from rms_locations l inner join inregion r on l.sbucod=r.sbucod and l.rgncod=r.rgncod  " + 
					"where r.sbucod='450' and r.rgncod in ("+zoneCodes+") ";
		}
		
		if(userType.equals("ZONE")) {
			sql="select l.loccod from rms_locations l inner join inregion r on l.sbucod=r.sbucod and l.rgncod=r.rgncod  " + 
					"where r.sbucod='450' and r.zoncod in ("+zoneCodes+") ";
		}
		
		userLocList = jdbcTemplate.query(sql, new ResultSetExtractor<List<String>>() {
